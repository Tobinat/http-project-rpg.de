...
if(count _nearc > 0) exitwith { hint "Du kannst das nicht tun,wenn die Box draußen ist."; };

_toDo = _this select 1;
if(_toDo == "nameUpdate") exitwith {
	if ((closetime + 180) > time) exitwith {hint "Du musst warten.";};
	_newName = ctrlText 8331;
	hint format ["Neuer Name: %1",_newName];
	player setvariable ["shop",player,false];
	[player,_newName] remoteExec ["server_fnc_StoreUpdateName",2];
	deleteMarkerLocal "Geschäft geschlossen";
	sklepotw = 1;
};
...
if(_toDo == "update") then {
	...
	if(_newPrice > 999999) then { _newprice = 999999; hint "Preis MAX = 999 999 $";};
...
while{_currentShop < 4} do {
	...
	while { _n != _loop } do {
		...
		_list lbAdd format["%1 | Menge: %2 | Preis: $%3", _item select 1, ((shopcontent select _currentShop) select 1) select _n, ((shopcontent select _currentShop) select 2) select _n];
