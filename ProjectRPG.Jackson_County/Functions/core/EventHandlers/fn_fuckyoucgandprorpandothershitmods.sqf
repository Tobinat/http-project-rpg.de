...
//junky drink
if (_item IN ["np_coke","np_drpepper"]) then {playSound3D ["PRPG_Data\sounds\drink.ogg", player, false, getPosASL player, 6, 1, 45]; ["add","drink",50] call client_fnc_sustain; ["add","unhealthiness",6] call client_fnc_sustain; [_item,0] spawn client_fnc_removeitem; closeDialog 0; };
//intox system
if (_item IN ["np_beer"]) then {playSound3D ["PRPG_Data\sounds\drink.ogg", player, false, getPosASL player, 6, 1, 45]; player playmove "vvv_anim_drink"; client_intox = client_intox + 0.02; [] spawn client_fnc_intox; [_item,0] spawn client_fnc_removeitem; closeDialog 0; };
if (_item IN ["plp_bo_w_BottleTequila"]) then {playSound3D ["PRPG_Data\sounds\drink.ogg", player, false, getPosASL player, 6, 1, 45]; player playmove "vvv_anim_drink"; client_intox = client_intox + 0.04; [] spawn client_fnc_intox; [_item,0] spawn client_fnc_removeitem; closeDialog 0; };
if (_item IN ["plp_bo_w_BottleBitters","plp_bo_w_BottleLiqCream","plp_bo_w_BottleLiqOrange"]) then {playSound3D ["PRPG_Data\sounds\drink.ogg", player, false, getPosASL player, 6, 1, 45]; player playmove "vvv_anim_drink"; client_intox = client_intox + 0.05; [] spawn client_fnc_intox; [_item,0] spawn client_fnc_removeitem; closeDialog 0; };
if (_item IN ["plp_bo_w_BottleBlueCorazol","plp_bo_w_BottleGin"]) then {playSound3D ["PRPG_Data\sounds\drink.ogg", player, false, getPosASL player, 6, 1, 45]; player playmove "vvv_anim_drink"; client_intox = client_intox + 0.03; [] spawn client_fnc_intox; [_item,0] spawn client_fnc_removeitem; closeDialog 0; };
//junkfood&drink
if (_item IN
...
if (_item == "CG_MethBag90") then { ["meth",15] spawn client_fnc_useDrug; [_item,0] spawn client_fnc_removeitem; closeDialog 0; };
if (_item == "CG_MethBag80") then { ["meth",12] spawn client_fnc_useDrug; [_item,0] spawn client_fnc_removeitem; closeDialog 0; };
if (_item == "CG_MethBag70") then { ["meth",9] spawn client_fnc_useDrug; [_item,0] spawn client_fnc_removeitem; closeDialog 0; };
if (_item == "CG_MethBag60") then { ["meth",6] spawn client_fnc_useDrug; [_item,0] spawn client_fnc_removeitem; closeDialog 0; };
if (_item == "CG_MethBag50") then { ["meth",3] spawn client_fnc_useDrug; [_item,0] spawn client_fnc_removeitem; closeDialog 0; };
...
if (_item == "CG_heroin") then { ["heroin",30] spawn client_fnc_useDrug; [_item,0] spawn client_fnc_removeitem;  closeDialog 0; };
if (_item == "CG_cocaine") then { ["cocaine",10] spawn client_fnc_useDrug; [_item,0] spawn client_fnc_removeitem; closeDialog 0; };
//barriers
