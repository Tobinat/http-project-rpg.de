...
if(_arrayCount == 10) then {
	if(_amount == 1) then { if("kif_fish_oil" in magazines player) then { ["Verabreiche Fischöl",10,client_fnc_healTime,_user,"AinvPknlMstpSnonWnonDnon_medic_1",[_user,_myInjuries,_arrayCount,"kif_fish_oil"],"prpg_data\sounds\patdown1.ogg"] spawn client_fnc_dotask; } else { hint "Du hast nicht das gebrauchte Material!"; }; };
	if(_amount > 0) then {
		if ( (_myInjuries select 10) == 5 ) then {
			["Du kannst diese Krankheit nicht behandeln.", false] spawn domsg;
		} else {
			if(myJob == "EMS" || myjob == "Fire") then {
				["Antibiotika verabreichen",10,client_fnc_healTime,_user,"AinvPknlMstpSnonWnonDnon_medic_1",[_user,_myInjuries,_arrayCount,"CG_ATF_Bandage_i"],"prpg_data\sounds\patdown1.ogg"] spawn client_fnc_dotask; 
			} else { 
