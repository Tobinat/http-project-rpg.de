...
_totaltest = 4;
myvehicletesticle = "Smart_Blue" createVehicle (getmarkerpos "t_test_25");
...
while{_totaltest > 0} do {
	while{true} do {
		...
		_postest = getmarkerpos _marker;
		if(player distance _postest < 3000) exitwith {};
		sleep 1;
	};
	["Schaue dich um, um den nächsten Checkpoint zu finden!",false] spawn domsg;
	[_postest,"driver"] spawn client_fnc_jobmarker;
	_timer = 0;
	anwser = "Error";
	_shitcunt = 0;
	while{true} do {
		if(!client_seatbelt) exitwith { _answer = "Nicht angeschnallt!"; };
		if(player distance _postest < 15) exitwith { _answer = "Checkpoint Erfolgreich!"; _totalres = _totalres + 1; };
		if(speed player >= 75) exitwith { _answer = "Höchstegeschwindigkeit überschritten!"; (vehicle player) setVelocity [0, 0, 0]; };
		if(_timer > 240) exitwith { _answer = "Du hast zu lange gebraucht!"; };
		_timer = _timer + 1;
		uisleep 1;
	...
	playSound "racebeep1";
	[format["%1",_answer],false] spawn domsg;
	marker_on = false;
	sleep 3;
