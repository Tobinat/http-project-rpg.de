...
lbClear _list_criminals;
{
	...
	_list_criminals lbAdd format["ID: %1 (Level:%3) - Name: %2", _ID, _supect_name, _wantedlevel];
...
lbClear _list_vehicles;
{
	...
	_desc = _x select 2;
	_wantedLevel = _x select 6;
	_list_vehicles lbAdd format["ID: %1 (Level:%3) - Beschreibung: (%4)%2", _ID, _desc, _wantedlevel, _plate];
