...
if(_cop == 12) then {_house = getpos nearestObject [[9626,3578.7,0.00143814], "Land_vvv_np_maison1"]; _higherup = true; };  //SLSD Sheriff
if(_cop == 11) then { _house = getpos nearestObject [[9516.43,3526.09,0.00143814], "Land_vvv_np_maison1"]; _higherup = true; };  //SLSD Ass. Sheriff
if(_ems == 8) then { _house = getpos nearestObject [[9412.31,3479.1,0.00143814], "Land_vvv_np_maison1"]; _higherup = true; };  //SLFD Chief
if(_ems == 7) then { _house = getpos nearestObject [[9306.67,3430.66,0.00143814], "Land_vvv_np_maison1"]; _higherup = true; }; //SLFD Ass. Chief
if(_legal == 10) then { _house = getpos nearestObject [[9188.22,3371.8,0.00143814], "Land_vvv_np_maison1"]; _higherup = true; }; //oberster richter
if(_legal == 9) then { _house = getpos nearestObject [[9094.99,3324.16,0.00143814], "Land_vvv_np_maison1"]; _higherup = true; }; //ltd. staatsanwalt
...
if(count(_prisonArray) > 0) then {
	...
	if(_houselevel == 3) then {
		_nObject = nearestObjects [_house, ["Land_Ranch_DED_Ranch_02_F","Land_Ranch_DED_Ranch_01_F"], 30];
		_containerpos = (_nobject select 0) getrelpos [8,0];
		player setpos _containerpos;
	};
	...
	if(_houselevel == 10) then {
		_nObject = nearestObjects [_house, ["Land_HouseDoubleAL2","Land_HouseDoubleAL"], 30];
		_containerpos = (_nobject select 0) getrelpos [15,0];
		player setpos _containerpos;
	};
};
