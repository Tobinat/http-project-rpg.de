...
if(cash_in_hand < 500) exitWith { ["INFO: Ich brauche 500$ !", false] spawn domsg; };
...
_timer=45;
while {_timer > 0} do {
	hintsilent format ["%1 Sekunden bis zum Absprung...",_timer];
	uiSleep 1;
	_timer = _timer - 1;
};
...
if(life_karma > 100) then {
	["Du erhälst einen 10% Rabatt wegen deinem guten Ruf!", false] spawn domsg;
...
["Du bist gelandet und dir steht ein Taxi zur Verfügung!", false] spawn domsg;
