...
if(!life_is_arrested) exitWith { 
	titleText["Das kann ich nicht machen","PLAIN"];
	titleFadeOut 5;
}; 

titleText["Das dauert jetzt ein bisschen...warte kurz","PLAIN"];
titleFadeOut 360;
_cme = 1;
...
if(!life_action_inUse) then {
	while {true} do {
		...
		if( player distance _myposy > 1.8 ) exitwith { 
				titleText["Du hast dich zu weit weg bewegt","PLAIN"];
					life_action_inUse = false;
		};
		if(life_istazed) exitwith {
			life_action_inUse = false;
			titleText["Du wurdest getasert...","PLAIN"];
		};
		if(player getVariable ["restrained", false]) exitwith {
			life_action_inUse = false;
			titleText["Du wurdest festgesetzt...","PLAIN"];
		};
		...
		if(_cme > 60) exitwith {
			...
			_id = player addAction ["Werkzeug Fallenlassen", {detach (_this select 3)},_veh];
			...
			titleText["Du hast dir Werkzeug hergestellt","PLAIN"];
		};
		life_action_inUse = false;
	};
	life_action_inUse = false;
