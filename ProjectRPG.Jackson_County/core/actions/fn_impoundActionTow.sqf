...
if (typeOf _vehicle IN ["critgamin_vangmcc_clothing","critgamin_vangmcc_fedex","critgamin_vangmcc_food","critgamin_vangmcc_general","critgamin_vangmcc_fixit","critgamin_vangmcc_pizza"]) exitwith
{
	...
	["Du wurdest für das Beschlagnahmen bezahlt: $44", false] spawn domsg;
};

if(!license_civ_truck) exitwith { ["Du hast keine LKW Lizenz.", false] spawn domsg; };

_towtrucks = nearestObjects [player, ["A3L_Towtruck"], 20];

if(count _towtrucks == 0) exitWith { ["Es befindet sich kein Abschlappwagen in der Nähe!", false] spawn domsg; };

if((player distance (getMarkerPos "tow_truck")) > 12) exitwith { ["Sie sind zu weit vom Schrottplatz entfernt!", false] spawn domsg; };

if(player distance _vehicle > 7.5) exitWith { ["Sie sind zu weit vom Fahrzeug entfernt!", false] spawn domsg; };
...
{	
	...
	if((getPlayerUID player) == _vehOwner) exitWith { ["Du kannst dein eigenes Fahrzeug nicht beschlagnahmen!", false] spawn domsg; };
	if(_vehicle getVariable["beingImpounded",false]) exitWith { ["Jemand anderes beschlagnahmt dieses Fahrzeug.", false] spawn domsg; };
	...
	if(deadPlayer) exitWith {["Du bist tot.", false] spawn domsg; life_action_inUse = false; _vehicle setVariable["beingImpounded",nil,true];};
