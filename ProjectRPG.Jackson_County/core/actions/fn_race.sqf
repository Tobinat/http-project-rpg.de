...
if((racemachine getVariable "racefull")) exitWith {
	["Dieses Rennen ist voll, mach bei dem Nächsten mit!", false] spawn domsg;
};	
...
if(!(racemachine getVariable "start")) then {
		[1,format["LAKESIDE RACING: Ein Rennen wird in 5 Minuten an der Rennstrecken in Lakeside beginnen]] remoteExecCall ["life_fnc_broadcast", civilian];
...
if(koildeb == 8) exitWith {
	["Dieses Rennen ist voll, mach beim nächsten mit!", false] spawn domsg;
};
...
uiSleep 1;
[0,format["%1 hatte eine Top-Runde von %2.",name player, _fastestlap]] remoteExecCall ["life_fnc_broadcast", -2];

	if(_laps > 0) then {
		["Das Rennen ist vorbei -  du bekommst $1000.", false] spawn domsg;
		["cash","add",1000] call life_fnc_handleCash;
	} else {
