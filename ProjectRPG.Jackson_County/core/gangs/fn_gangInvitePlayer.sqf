...
if(!isNil {(group _unit) getVariable "gang_name"}) exitWith {hint "Dieser Spieler ist bereits in einer Gang!"}; //Added

if(count(grpPlayer getVariable ["gang_members",8]) isEqualTo (grpPlayer getVariable ["gang_maxMembers",8])) exitWith {hint localize "STR_GNOTF_MaxSlot"};
