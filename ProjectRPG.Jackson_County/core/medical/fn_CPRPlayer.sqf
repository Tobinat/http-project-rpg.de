...
if(_target getVariable ["CPR",ObjNull] == player) exitWith {["Someone is already performing CPR..", false] spawn domsg;};
...
if(_chance < 200) then {
	...
	sleep 2;
	if(_target getVariable["Revive",FALSE]) then { ["You revived this player", false] spawn domsg; } else { ["You stabilized this player", false] spawn domsg; };
