...
[0,format["%1 wurde durchsucht",name player, _cop]] remoteExecCall ["life_fnc_broadcast", -2]; 

if(turtle_fuck == 2) then {
	[0,format["Bei %1 wurden %2 seltene Schildkröten entdeckt",name player, totalturtles]] remoteExecCall ["life_fnc_broadcast", -2]; 
};
if(drug_fuck == 2) then {
	[0,format["Bei %1 wurden %2 illegale Rauschmittel sichergestellt",name player, totaldrugs]] remoteExecCall ["life_fnc_broadcast", -2]; 
} else {
	[0,format["%1 hat keine illegalen Rauschmittel bei sich",name player]] remoteExecCall ["life_fnc_broadcast", -2]; 
};
...
[localize "Durchsuchung abgeschlossen...Alle illegalen Gegenstände wurden abgenommen", false] spawn domsg;
