...
switch (playerSide) do
{
	...
	{
		//Ausweis
		life_actions = life_actions + [player addAction["<t color='#00FF00'>Dienstausweis zeigen</t>",life_fnc_copShowLicense,"",1,false,true,"",' playerSide == independent && !isNull cursorTarget && player distance cursorTarget < 2.5 && cursorTarget isKindOf "Man" && alive cursortarget ']];
		...
		//Person Heilen
		life_actions pushBack (player addAction["Selber Heilen",life_fnc_healself,"",0,false,false,"",' (player getVariable "playerHealth") > 0 && !isdragging ']);

		//Heal Self
		life_actions pushBack (player addAction["Person Heilen",life_fnc_healperson,"",0,false,false,"",' !isNull cursorTarget && player distance cursorTarget < 2.5 && isPlayer cursorTarget && (cursorTarget getVariable "playerHealth") > 0 && !isdragging ']);
	...
	case license_civ_udc:
	{
		...
		//Ausweis
		life_actions = life_actions + [player addAction["<t color='#00FF00'>Ausweis Zeigen</t>",life_fnc_copShowLicense,"",1,false,true,"",'
	...
	//Ausweis
		...
		//place roadblock
		life_actions pushBack (player addAction["Place Roadblocfk",{if(!isNull life_roadblock) then {{detach _x;} foreach (life_roadblock getvariable "roadblock"); 
		_handle = [life_roadblock,"roadblock"] spawn life_fnc_enablecollisionwith; waitUntil {scriptDone _handle}; [life_roadblock,"roadblock"] remoteExec ["life_fnc_enablecollisionwith"]; life_roadblock = ObjNull;};},"",999,false,false,"",'!isNull life_roadblock']);
	...
	{
		//Ausweis
		life_actions = life_actions + [player addAction["<t color='#00FF00'>Polizeimarke Zeigen</t>",life_fnc_copShowLicense,"",1,false,true,"",' playerSide == west && !isNull cursorTarget && player distance cursorTarget < 2.5 && cursorTarget isKindOf "Man" && alive cursortarget ']];
