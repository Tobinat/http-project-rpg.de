...
["You can not do that.", false] spawn domsg;
titleFadeOut 5;
}; 

["Das wird einige Zeit beanstpruchen... Warten Sie kurz.", false] spawn domsg;
titleFadeOut 20;
_cme = 1;
...
_cme = _cme + 1;
	if( player distance _myposy > 1.8 ) exitwith { 
			["Du bist zu Weit entfernt.", false] spawn domsg;
	};
	if(_cme > 20) exitwith {
		...
		titleText["Du braucht ein Seil um Leute zu Fesseln!","PLAIN"];
	};
	life_action_inUse = false;
};
