...
if(_type == "Karma") then {
	...
	if(_adjust == "Add") then { client_karma = client_karma + _amount; [format["You just gained %1 reputation! Total: %2",_amount,client_karma], true] spawn domsg; };
	if(_adjust == "Remove") then { client_karma = client_karma - _amount; [format["You just lost %1 reputation! Total: %2",_amount,client_karma], true] spawn domsg; };
...
if(_type == "unhealthiness") then {
	if(_adjust == "Add") then { [format["+%1 Unhealthiness",_amount],true] spawn domsg; client_unhealthiness = client_unhealthiness + _amount; };
	if(_adjust == "Remove") then { [format["-%1 Unhealthiness",_amount],true] spawn domsg; client_unhealthiness = client_unhealthiness - _amount; };
	...
	if(client_unhealthiness > 60) then {
		hint "You are currently very unhealthy";
...
if(_type == "license") then {
	...
	if(_adjust == "Add") then { 
		...
		if(_amount == 1) then { licensearray SET [0,1]; ["You got a Drivers License",false] spawn domsg; };
		if(_amount == 2) then { licensearray SET [1,1]; ["You got a Gun License",false] spawn domsg;  };
		if(_amount == 3) then { licensearray SET [2,1]; ["You got a Mining License",false] spawn domsg;  };
		if(_amount == 4) then { licensearray SET [3,1]; ["You got a Loggers License",false] spawn domsg; };
		if(_amount == 5) then { licensearray SET [4,1]; ["You got a Fishing License",false] spawn domsg; };

	};

	if(_adjust == "Remove") then { 

		if(_amount == 1) then { licensearray SET [0,0]; ["You lost a Drivers License",false] spawn domsg; };
		if(_amount == 2) then { licensearray SET [1,0]; ["You lost a Gun License",false] spawn domsg; };
		if(_amount == 3) then { licensearray SET [2,0]; ["You lost a Mining License",false] spawn domsg; };
		if(_amount == 4) then { licensearray SET [3,0]; ["You lost a Loggers License",false] spawn domsg; };
		if(_amount == 5) then { licensearray SET [4,0]; ["You lost a Fishing License",false] spawn domsg; };
