...
if(_shop IN robbedstores) exitwith { hint "Hier gibt es nix zu holen!"; };
...
if(_chance > 90) then {
	...
	if(modifier == 0) then {
		...
		while {true} do {
			...
			if(modifier == 0) exitwith { hint "Deine Laden Radale haben aufgehört"; };
			uisleep 20;
			modifier = modifier - 1;
		...
		robbedstores pushback _shop; hint format["Du hast diesen Laden gerade um %1 beraubt!",_amount]; [_amount] call Client_fnc_addCash;
