	...
	// if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && count _sPos > 1 && _distance < 7 && isPlayer _x) then {
		...
		// if(_x getVariable ["tf_isSpeaking",false] && !(_x getVariable ["dead",false])) then {
			// if(!isNil {(group _x) getVariable "gang_name"}) then {
				...
				// _text = format["<t color='#FFFFFF' font='puristaMedium'>%1</t> <img size='1' image='icons\smalltalk.paa'/> <br/><t size='0.8' font='puristaMedium' color='#B6B6B6'>%2</t>",_name,_groupname];
			// } else {
				// _text = format["<t color='#FFFFFF' font='puristaMedium'>%1</t> <img size='1' image='icons\smalltalk.paa'/>",_name];								
			// };

			// if (goggles _x IN GoggleList || headgear _x IN HatList ) then {
				// _text = format["<t color='#FFFFFF' font='puristaMedium'>Masked</t> <img size='1' image='icons\smalltalk.paa'/>"];				
			// };
			...
			// if(side player == independent) then {
				// if ((_x getVariable["severeInjuries",FALSE]) || _x getVariable ["diseaseon",1] > 1 || _x getVariable ["unconciouson",1]  > 1 || _x getVariable ["fractureon",1]  > 1 || _x getVariable ["bleedingon",1]  > 1 || _x getVariable ["woundedon",1] > 1) then {
					// _text = "<img size='0.2' image='icons\drugs.paa'/><t color='#FFFFFF' font='puristaMedium'>Injured</t>";
				// };
			// };
