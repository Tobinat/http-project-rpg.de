...
waitUntil {sleep 1; !alive _vehicle};_vehicle setVariable["dbInfo",[],true];sleep 10;if(!isNull _vehicle && !isNil "_vehicle") then {deleteVehicle _vehicle;};if(count _dbInfo > 0) then {	_query = format["UPDATE vehicles SET active='0', alive='1' WHERE pid='%1' AND plate='%2'",_uid,_plate];	waitUntil {!DB_Async_Active};	_sql = [_query,1] call DB_fnc_asyncCall;}; 