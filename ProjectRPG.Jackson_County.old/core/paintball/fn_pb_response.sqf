...
if (_respondmode==2 && !deadPlayer) then {
    ...
    hint "Herzlich Willkommen in der Paintball Arena! Das Spiel hat eine Länge von 5 Minuten.";
    ...
    if (vehicle player != player) then {
        ...
        hint "Ein Paintball Match beginnt, steige aus um daran Teilzunehmen.";
...
if (_respondmode==3) then {
    ...
    if (tode>=_grenzeAusz) then {
        hint "Das Spiel wurde beendet. Leider hast du nicht genügend Punkte für das Auszahlen des Preisgeldes!";
    } else {
        ["cash","add",3000] call life_fnc_handleCash; 
        hint "Du hast $3000 gewennon!";
    };
};
