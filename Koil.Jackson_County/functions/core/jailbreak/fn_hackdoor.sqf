...
_fencetoopen = cursorobject;
	hint parsetext "<img size='1' image='CG_Jobs\icons\info.paa'/> <t color='#FFCC00'><t size='0.75'>Starte Hack</t><br/> Zugangspunkt wird geöffnet.";
	...
	while {true} do {
		...
		uisleep 1;
		hint parsetext format["<img size='1' image='CG_Jobs\icons\info.paa'/> <t color='#FFCC00'><t size='0.75'>Modifiziere Sicherheits Zelle</t><br/> %1 von 25 Sekunden.",_calcT];
	};
	...
	if(typeOf _fenceToOpen == "Land_GateB") then {
		hint parsetext format["<img size='1' image='CG_Jobs\icons\info.paa'/> <t color='#FFCC00'><t size='0.75'>Daten modifiziert</t><br/> Zugangspunkt geöffnet.",_calcT];
		_fenceToOpen animate ['GateDoor_3',-1.6];
	};

	if(typeOf _fenceToOpen == "Land_Gaol_Main") then {

		hint parsetext format["<img size='1' image='CG_Jobs\icons\info.paa'/> <t color='#FFCC00'><t size='0.75'>Sekundäf Modul ist gehackt</t><br/> Sie haben vollen Zugriff.",_calcT];
	...
	if(typeOf _fenceToOpen == "Land_MainSection") then {
		...
		hint parsetext format["<img size='1' image='CG_Jobs\icons\info.paa'/> <t color='#FFCC00'><t size='0.75'>Haupt Modul ist gehackt</t><br/> Sie haben vollen Zugriff.",_calcT];
