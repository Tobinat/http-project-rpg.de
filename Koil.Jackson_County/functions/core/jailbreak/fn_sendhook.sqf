...
if((_height select 2) > 15) exitwith { hint "Du musst auf dem Boden sein um einzuklinken!"; };
...
if(count tower > 0) then {
	if(player distance (tower select 0) < 16) then { hint "Du bist zu nah"; };
	godMode = true;
	_speed = 115;
	...
	hint "Du bist zu weit weg um auf dem Tower einzuklinken!";
};